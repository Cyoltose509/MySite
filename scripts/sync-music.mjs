...
async function main() {
  ...
  // 5. Sync covers to Supabase
  log('\x1b[34m→\x1b[0m', '同步专辑封面...');

  const coverList = tracks.filter(t => t.picUrl).map(t => [t.netease_id, t.picUrl]);
  if (coverList.length > 0) {
    for (let i = 0; i < coverList.length; i += 100) {
      const chunk = coverList.slice(i, i + 100);
      const ph = chunk.map((_, j) => `($${j*2+1},$${j*2+2})`).join(',');
      const params = chunk.flat();
      await client.query(
        `INSERT INTO music_covers (netease_id, cover_url) VALUES ${ph} ON CONFLICT (netease_id) DO UPDATE SET cover_url = EXCLUDED.cover_url`,
        params
      );
    }
  }
  log('\x1b[32m✓\x1b[0m', `封面已同步 (${coverList.length} 首)`);
