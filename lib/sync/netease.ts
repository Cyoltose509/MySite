      ...
      for (const s of detailJson.songs) {
        tracks.push({
          ...
          duration: (s.duration || s.dt || 0) > 1000
            ...
            : (s.duration || s.dt || 0),
          tags: [],
          picUrl: s.album?.picUrl || s.al?.picUrl || undefined,
        });
      }
    }
...
export async function syncCoversToSupabase(
  tracks: NeteaseTrack[],
): Promise<void> {
  const { supabase } = require('@/lib/supabase');
  const covers = tracks.filter(t => t.picUrl);
  for (let i = 0; i < covers.length; i += 100) {
    const chunk = covers.slice(i, i + 100);
    await supabase.from('music_covers').upsert(
      chunk.map(c => ({ netease_id: parseInt(c.id), cover_url: c.picUrl })),
      { onConflict: 'netease_id' }
    );
  }
}
