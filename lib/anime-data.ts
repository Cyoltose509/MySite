...
export interface AnimeItem {
  ...
  premiereDate?: string; // YYYY-MM-DD from frontmatter
}
...
function parseAnimeMarkdown(text: string, filepath: string): AnimeItem | null {
  ...
  return { title, status, rating, tags: cleanTags, source, body, filePath: filepath, premiereDate: extractDate(fm) };
}

function extractDate(fm: string): string | undefined {
  const m = fm.match(/^(?:premiere_date|date):\s*(.+)/im);
