...
export function MoodLogger() {
  ...
  const handleSave = async () => {
    ...
    try {
      let result;
      if (editingId) {
        result = await supabase.rpc('fn_update_mood_log', {
          p_hash: hash,
          p_log_id: editingId,
      ...
      if (result.error || d?.error) {
        setMessage({ text: `❌ ${result.error?.message || d.error}`, type: 'err' });
      } else {
        setMessage({ text: editingId ? '✅ 心情记录已更新' : '✅ 心情记录已保存', type: 'ok' });
        resetForm();
        fetchLogs();
      }
    } catch (e: any) {
    ...
    <div style={styles.card}>
      ...
      <div style={styles.formSection}>
        ...
        <div style={styles.row}>
          ...
          <select value={visibility} onChange={(e) => setVisibility(e.target.value as 'public' | 'private')} style={{...styles.select, flex: 0.5}}>
