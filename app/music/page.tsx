...
export default function MusicPage() {
  ...
  const fetchMusic = async () => {
    ...
    // Load covers from Supabase
    const { data: coverData } = await supabase.from('music_covers').select('*');
    if (coverData) {
      const covers: Record<string, string> = {};
      for (const c of coverData) covers[String(c.netease_id)] = c.cover_url;
      setCoverMap(covers);
    }

    // Load entity refs for all music (both directions)
    const { data: refsOut } = await supabase.from('entity_refs').select('*').eq('source_type','music');
    const { data: refsIn } = await supabase.from('entity_refs').select('*').eq('target_type','music');
    const refsData = [...(refsOut||[]), ...(refsIn||[])];
    if (refsData.length > 0) {
      const rm: Record<string, {anime:string[];games:{id:string;title:string}[]}> = {};
      for (const r of refsData) {
        const musicId = r.source_type === 'music' ? r.source_id : r.target_id;
        const otherType = r.source_type === 'music' ? r.target_type : r.source_type;
        const otherId = r.source_type === 'music' ? r.target_id : r.source_id;
        if (!rm[musicId]) rm[musicId] = {anime:[],games:[]};
        if (otherType === 'anime') rm[musicId].anime.push(otherId);
        else if (otherType === 'game') rm[musicId].games.push({id:otherId,title:otherId});
      }
      // Resolve game titles
      const { data: games } = await supabase.from('steam_games').select('id,title');
      if (games) {
        for (const [mid, refs] of Object.entries(rm)) {
          refs.games = refs.games.map(g => {
            const found = games.find(gg => gg.id === g.id);
            return found ? {id:g.id, title:found.title} : g;
          });
        }
      }
      setRefsMap(rm);
      ...
      {detailMusic && (
        <div style={modalOverlayStyle} onClick={() => setDetailMusic(null)}>
          <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
            ...
            {/* Entity refs */}
            {(() => {
              const refs = refsMap[detailMusic.id];
              const sc = singCounts[detailMusic.id];
              if ((!refs || (!refs.anime.length && !refs.games.length)) && !sc) return null;
              return (
                <div style={{ display:'flex', gap:8, flexWrap:'wrap', padding: '8px 0', borderTop: '1px solid rgba(255,255,255,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)', marginBottom:12 }}>
                  {refs?.anime.map(a => (
                    <a key={a} href={`/anime?search=${encodeURIComponent(a)}`} target="_blank" style={{textDecoration:'none'}} rel="noreferrer">
                      <span style={{fontSize:11,color:'#c084fc',padding:'3px 8px',borderRadius:6,background:'rgba(168,85,247,0.1)',cursor:'pointer'}}>
                        📺 {a}
                      </span>
                    </a>
                  ))}
                  {refs?.games.map(g => (
                    <a key={g.id} href="/games" target="_blank" style={{textDecoration:'none'}} rel="noreferrer">
                      <span style={{fontSize:11,color:'#4ade80',padding:'3px 8px',borderRadius:6,background:'rgba(74,222,128,0.1)',cursor:'pointer'}}>
                        🎮 {g.title}
                      </span>
                    </a>
                  ))}
